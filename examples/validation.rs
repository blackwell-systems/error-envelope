...
async fn create_user(Json(req): Json<CreateUserRequest>) -> Result<Json<User>, Error> {
    ...
    if req.email.len() < 5 {
        errors.insert("email".to_string(), "must be at least 5 characters".to_string());
    }
    ...
    if !req.password.chars().any(|c| c.is_numeric()) {
        errors.insert("password".to_string(), "must contain at least one number".to_string());
    }
    ...
    if req.username.len() < 3 {
        errors.insert("username".to_string(), "must be at least 3 characters".to_string());
    }
    if !req.username.chars().all(|c| c.is_alphanumeric() || c == '_') {
        errors.insert("username".to_string(), "can only contain letters, numbers, and underscores".to_string());
    }
...
async fn main() {
    ...
    println!(r#"  curl -X POST http://localhost:3000/user \
